		...
		if( opts["print-results"] ) {
			...
			if( typeof res["status_code"] === "string" && 
				...
				for( i = 0; i < res.results.length; i++ ) {
					console.log(res["results"][i]["local_id"]);
					if( res["results"][i]["status_code"] === "OK" ) {
						for (j = 0; j < 5; j++) {
							console.log(res["results"][i].result["tag"]["classes"][j] );
						}
					console.log('\n');
...
function exampleTagSingleURL() {
	var testImageURL = 'https://upload.wikimedia.org/wikipedia/commons/e/eb/Ash_Tree_-_geograph.org.uk_-_590710.jpg';
	var ourId = "tree 1"; // this is any string that identifies the image to your system
...
function exampleTagMultipleURL() {
	var testImageURLs = [ 
	"https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcTmAfV-7DRQlfNxwRN9CvfCuxEYXetz_gffo-zHQhRUKIUBL-Q37w", 
	"http://cdn.v-harness.com/wp-content/uploads/2015/03/tree.png" ];
	var ourIds =  [ "tree 2", 
	                "tree 3" ]; // this is any string that identifies the image to your system
